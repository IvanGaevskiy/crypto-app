class Api::ExchangeTransactionsController < ApplicationController
  def show
    @exchangeTransaction = ExchangeTransaction.new(transaction_params)

    exchange_rates_params.each do |rate_params|
      @exchangeTransaction.exchange_rate.build(rate_params)
    end

    agreetments_params.each do |agreetment_param|
      @exchangeTransaction.agreetment.build(agreetment_param)
    end
